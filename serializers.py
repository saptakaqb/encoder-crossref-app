...
def _make_display_code(
    ...
    EPC:    Strip internal 'EPC-' prefix and fill the 'XXXX' PPR placeholder
            with the first matched PPR value (zero-padded to 4 digits).
            e.g. 'EPC-TR1-20-T1-XXXX-A-PP-R4-F00-S2' + cpr_covered=[1024]
                 -> 'TR1-20-T1-1024-A-PP-R4-F00-S2'

    Kübler: Synthetic Bronze2 codes (KIS40-XXXXX-C-5-...) are internal and
            do not resemble real Kübler order codes.  Show family name only
            until the Kübler display-code enrichment pipeline is run.
            e.g. 'KIS40-XXXXX-...' -> 'KIS40'
    ...
    if "encoder products" in mfr or mfr == "epc":
        ...
        if code.upper().startswith("EPC-"):
            code = code[4:]                       # strip internal prefix
    ...
    if "kubler" in mfr or "kübler" in mfr:
        # Synthetic codes are meaningless to engineers — show family until
        # enrich_kubler_display_codes.py has been run.
...
def serialize_result(
    ...
    display_code = _make_display_code(manufacturer, part_number, family, cpr_covered)
